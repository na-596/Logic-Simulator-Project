...
class MyGLCanvas(wxcanvas.GLCanvas):
    ...
    def __init__(self, parent, devices, monitors):
        ...
        self.scroll_sensitivity = 0.01  # Adjust this to change scrolling speed
    ...
    def render(self, text=""):
        ...
        size = self.GetClientSize()
        max_time_units = max(
            int(self.get_viewport_width() / 30),
            max((len(i) for i in self.signal_data.values()), default=0)
        )
        num_signals = len(self.signal_data)
        total_height = max(
            (num_signals + 1) * self.signal_height,
            size.height - self.bottom_margin
        )
    ...
    def draw_signals(self):
        ...
        GL.glLineWidth(self.signal_line_width)  # Set thicker line for signals
        ...
        for i, (signal_name, values) in enumerate(self.signal_data.items()):
            ...
            first_valid_found = False  # Track if first valid signal
            ...
            for t, value in enumerate(values):
                ...
                if value:
                    y = y_base + self.signal_height * 0.8
                else:
                    y = y_base + self.signal_height * 0.2

                # Draw vertical line if value changed and
                # neither current nor previous is blank
                # only if this isn't the first valid signal we've encountered
                if (
                    value != last_valid_value and
    ...
    def get_max_scroll_x(self):
        ...
        content_width = (
            (max_time_units - (self.get_viewport_width() / 30)) * 15
        )
    ...
    def scroll_to_time(self, time_unit):
        ...
        if target_x < current_min_x or target_x > current_max_x:
            ...
            self.pan_x = max(
                -self.get_max_scroll_x() * self.zoom,
                min(self.pan_x, 0)
            )
            self.init = False
            self.Refresh()
...
class Gui(wx.Frame):
    ...
    on_stop_button(self, event): Event handler for when the user clicks the
                                 stop button.

    on_reset_button(self, event): Event handler for when the user clicks the
                                reset button.
    ...
    def __init__(
            self, title, path, names, devices,
            network, monitors, language
    ):
        ...
        self.light_theme = {
            ...
            'static_box': {
                # Slightly darker than control panel
                'background': wx.Colour(240, 240, 240),
                'text': wx.Colour(0, 0, 0)
            },
            'switch': {
        ...
        self.dark_theme = {
            ...
            'button': {
                'run': wx.Colour(0, 184, 148),        # Green color: run button
            ...
            'canvas': {
                ...
                'grid': wx.Colour(48, 49, 52)        # lighter grid lines
            },
            'list': {
                # lighter than background
                'background': wx.Colour(41, 42, 45),
                'text': wx.Colour(232, 234, 237),  # Google Light Grey Text
                # Dark header background
                'header_background': wx.Colour(35, 36, 39),
                # Light text for header
                'header_text': wx.Colour(232, 234, 237),
                'alternate': wx.Colour(45, 46, 49)     # Alternate row color
            },
            'static_box': {
                # Slightly darker than control panel
                'background': wx.Colour(28, 29, 32),
                # Google Light Grey Text
                'text': wx.Colour(232, 234, 237)
            },
            'switch': {
                'on': wx.Colour(0, 184, 148),  # Green
        ...
        self.light_signal_colors = [
            ...
            (0.95, 0.80, 0.10),  # Hard Yellow (a rich, golden yellow)
        ...
        self.dark_signal_colors = [
            (1.00, 0.05, 0.65),  # Neon Magenta (a vibrant, pink to kick off)
        ...
        self.light_mode_item = self.themeMenu.AppendRadioItem(
            self.LIGHT_MODE_ID, wx.GetTranslation("Light Mode")
        )
        self.dark_mode_item = self.themeMenu.AppendRadioItem(
            self.DARK_MODE_ID, wx.GetTranslation("Dark Mode")
        )
        ...
        # Simulation controls
        self.sim_box = wx.StaticBox(
            self.control_panel, label=wx.GetTranslation("Simulation Controls")
        )
        ...
        self.speed_btn = wx.Button(
            self.control_panel, label=self.current_speed, size=(60, 25)
        )
        speed_label = wx.StaticText(
            self.control_panel, label=wx.GetTranslation("Speed:")
        )
        ...
        # Cycles control
        cycles_label = wx.StaticText(
            self.control_panel, label=wx.GetTranslation("Number of Cycles:")
        )
        self.cycles_spin = wx.SpinCtrl(
            self.control_panel, value="10", min=1, max=1000
        )
        ...
        self.run_button = wx.Button(
            self.control_panel,
            label=wx.GetTranslation("▶ Run Simulation"),
            style=button_style
        )
        self.stop_button = wx.Button(
            self.control_panel,
            label=wx.GetTranslation("❚❚ Pause"),
            style=button_style
        )
        self.reset_button = wx.Button(
            self.control_panel,
            label=wx.GetTranslation("■ Reset"),
            style=button_style
        )
        self.continuous_button = wx.Button(
            self.control_panel,
            label=wx.GetTranslation("∞ Continuous"),
            style=button_style
        )
        ...
        self.continuous_button.SetBackgroundColour(wx.Colour(255, 255, 255))
        ...
        self.switch_box = wx.StaticBox(
            self.control_panel, label=wx.GetTranslation("Switch Controls")
        )
        ...
        self.switch_list = CustomListCtrl(
            self.control_panel, style=wx.LC_REPORT, gui=self
        )
        self.switch_list.InsertColumn(
            0, wx.GetTranslation("Switch"), width=150
        )  # Switch name column
        # State column
        self.switch_list.InsertColumn(1, wx.GetTranslation("State"), width=80)
        ...
        # Add toggle button
        self.toggle_switch_btn = wx.Button(
            self.control_panel, label=wx.GetTranslation("Toggle Selected")
        )
        # Initially disabled until switches are selected
        self.toggle_switch_btn.Disable()

        # Add toggle buttons for all on/off
        self.all_on_btn = wx.Button(
            self.control_panel, label=wx.GetTranslation("All On")
        )
        self.all_off_btn = wx.Button(
            self.control_panel, label=wx.GetTranslation("All Off")
        )
        ...
        self.monitor_box = wx.StaticBox(
            self.control_panel, label=wx.GetTranslation("Monitors")
        )
        ...
        self.monitor_list = CustomListCtrl(
            self.control_panel, style=wx.LC_REPORT, gui=self
        )

        # Narrow column for color indicator
        self.monitor_list.InsertColumn(0, "", width=20)
        self.monitor_list.InsertColumn(
            1, wx.GetTranslation("Signal"), width=140
        )  # Device name column
        # State column, narrower
        self.monitor_list.InsertColumn(2, wx.GetTranslation("State"), width=55)
        # Zap button column, even wider for scroll bar
        self.monitor_list.InsertColumn(3, "", width=45)

        # Set column widths
        # Narrow column for color indicator
        self.monitor_list.SetColumnWidth(0, 20)
        # Fixed width for device name
        self.monitor_list.SetColumnWidth(1, 140)
        # Fixed width for state, narrower
        self.monitor_list.SetColumnWidth(2, 55)
        # Fixed width for zap button, even wider for scroll bar
        self.monitor_list.SetColumnWidth(3, 45)
        ...
        self.add_monitor_btn = wx.Button(
            self.control_panel, label=wx.GetTranslation("Add Monitor(s)")
        )
        self.remove_monitor_btn = wx.Button(
            self.control_panel, label=wx.GetTranslation("Zap All")
        )
        ...
        self.monitor_list.Bind(
            wx.EVT_LIST_ITEM_SELECTED, self.on_monitor_selected
        )
        self.monitor_list.Bind(
            wx.EVT_LIST_ITEM_DESELECTED, self.on_monitor_selected
        )
        ...
        self.switch_list.Bind(
            wx.EVT_LIST_ITEM_SELECTED, self.on_switch_selected
        )

        # Add accelerator table for F1 (Help), Alt+F4 (Exit),
        # and Spacebar (Run/Pause)
        self.SPACEBAR_ID = wx.NewId()
        accel_tbl = wx.AcceleratorTable([
            (wx.ACCEL_NORMAL, wx.WXK_F1, wx.ID_HELP),
            (wx.ACCEL_ALT, wx.WXK_F4, wx.ID_EXIT),
            (wx.ACCEL_NORMAL, wx.WXK_SPACE, self.SPACEBAR_ID),
        ])
        ...
        if Id == wx.ID_HELP:
            wx.MessageBox(
                wx.GetTranslation(
                    ...
                    "**Simulation Controls**\n"
                    "- **Number of Cycles:** Set how many cycles to run the "
                    "simulation for.\n"
                    "- **▶ Run Simulation:** Start the simulation for the "
                    "chosen number of cycles.\n"
                    "- **❚❚ Pause:** Pause the simulation at any time.\n"
                    "- **■ Reset:** Reset the simulation and clear all "
                    "monitor data.\n"
                    "- **Speed:** Click to cycle through simulation speeds "
                    "(x0.5, x1, x2, x4, x8).\n\n"
                    "**Switch Controls**\n"
                    "- Toggle individual switches by clicking the switch in "
                    "the list.\n"
                    "- Use **All On** or **All Off** to set all switches "
                    "high or low.\n\n"
                    "**Monitors**\n"
                    "- Add a monitor to track a signal by clicking "
                    "**Add Monitor** and selecting a signal.\n"
                    "- Use **Add All** in the dialog to monitor all "
                    "available signals.\n"
                    "- Remove a monitor by clicking the ✕ button next to it, "
                    "or **Zap All** to remove all monitors.\n"
                    "- The monitor list shows the current state of each "
                    "monitored signal.\n\n"
                    "**Signal Display Canvas**\n"
                    "- The main area shows waveforms for all monitored "
                    "signals.\n"
                    "- **Pan:** Click and drag to move the view.\n"
                    "- **Zoom:** Use the mouse wheel to zoom in/out.\n"
                    "- Signal colors match the color bars in the monitor "
                    "list.\n\n"
                    "**Themes**\n"
                    "- Switch between Light and Dark mode from the Theme "
                    "menu.\n\n"
                    "**Status Bar**\n"
                    "- The status bar at the bottom shows helpful messages "
                    "and feedback.\n\n"
                    "For more help, see the project documentation or contact "
                    "the authors."
                ),
    ...
    def start_simulation(self, num_cycles):
        ...
        if not self.is_running:
            ...
            self.SetStatusText(
                wx.GetTranslation(
                    "Running simulation for {num_cycles} cycles..."
                ).format(num_cycles=num_cycles)
            )
        ...
        if self.is_running and self.current_cycle < self.target_cycles:
            ...
            if self.execute_cycle():
                ...
                if self.current_cycle >= self.target_cycles:
                    self.stop_simulation()
                    self.SetStatusText(
                        wx.GetTranslation("Simulation completed")
                    )
            else:
                self.stop_simulation()
                self.SetStatusText(
                    wx.GetTranslation("Simulation error occurred")
                )
    ...
    def update_monitor_list(self, show_states=False):
        ...
        # Get monitored signals and maintain original order
        #  from monitors dictionary
        monitored_signals = []
        for i, (d_id, out_id) in enumerate(self.monitors.monitors_dictionary):
            signal_name = self.devices.get_signal_name(d_id, out_id)
            # Store original index to maintain color consistency
            monitored_signals.append((signal_name, d_id, out_id, i))
        ...
        # Add to list
        for list_index, (
                signal_name, device_id, output_id, original_index
            ) in enumerate(
                monitored_signals
        ):
            ...
            if list_index % 2 == 0:
                self.monitor_list.SetItemBackgroundColour(
                    index, self.current_theme['list']['background']
                )
            else:
                self.monitor_list.SetItemBackgroundColour(
                    index, self.current_theme['list']['alternate']
                )
            ...
            if show_states:
                ...
                current_signal = self.network.get_output_signal(
                    device_id, output_id
                )
            ...
            zap_button = wx.Button(
                self.monitor_list, id=index, label="✕",
                size=(25, 20), style=wx.NO_BORDER
            )
            zap_button.SetBackgroundColour(
                self.current_theme['button']['reset']
            )  # Red color
            ...
            zap_button.Bind(
                ...
                signal=signal_name: self.on_zap_button(evt, signal)
            )
            ...
            button_x = (
                rect.x +
                self.monitor_list.GetColumnWidth(0) +
                self.monitor_list.GetColumnWidth(1) +
                self.monitor_list.GetColumnWidth(2)
            )
    ...
    def on_zap_button(self, event, signal_name):
        ...
        for (device_id, output_id) in list(
            ...
            # Get the signal name for this device and output
            if signal_name == self.devices.get_signal_name(
                device_id, output_id
            ):
                # Remove the monitor
                if self.monitors.remove_monitor(device_id, output_id):
                    self.SetStatusText(
                        wx.GetTranslation(
                            "Zapped monitor for {signal_name}".format(
                    ...
                    wx.MessageBox(
                        wx.GetTranslation(
                            "Failed to zap monitor {signal_name}".format(
                                signal_name=signal_name
                            )
                        ),
    ...
    def update_signal_display(self):
        ...
        for (
            (device_id, output_id),
            signal_list
        ) in self.monitors.monitors_dictionary.items():
            ...
            for signal in signal_list:
                if (
                    signal == self.devices.HIGH or
                    signal == self.devices.RISING
                ):
                    binary_signals.append(1)
                elif (
                    signal == self.devices.LOW or
                    signal == self.devices.FALLING
                ):
    ...
    def on_add_monitor(self, event):
        ...
        dialog = wx.Dialog(
            self, title=wx.GetTranslation("Add Monitor"), size=(300, 150)
        )
        ...
        if not non_monitored:
            wx.MessageBox(
                wx.GetTranslation("No more signals available to monitor"),
                wx.GetTranslation("Information"),
        ...
        signal_label = wx.StaticText(
            dialog, label=wx.GetTranslation("Signal:")
        )
        ...
        ok_button = wx.Button(
            dialog, wx.ID_OK, wx.GetTranslation("Add")
        )
        add_all_button = wx.Button(
            dialog, wx.NewId(), wx.GetTranslation("Add All")
        )
        cancel_button = wx.Button(
            dialog, wx.ID_CANCEL, wx.GetTranslation("Cancel")
        )
        ...
        signal_choice.Bind(
            wx.EVT_CHOICE, lambda evt: ok_button.Enable(
                evt.GetSelection() != -1
            )
        )
        ...
        # Bind Add All button
        add_all_button.Bind(
            wx.EVT_BUTTON, lambda evt: self.on_add_all_monitors(evt, dialog)
        )

        if (
            dialog.ShowModal() == wx.ID_OK and
            signal_choice.GetSelection() != -1
        ):
    ...
    def add_single_monitor(self, signal_name):
        ...
        if self.monitors.monitors_dictionary:
            ...
            first_monitor = next(
                iter(self.monitors.monitors_dictionary.values())
            )
                ...
                if signal_name == self.devices.get_signal_name(
                    ...
                    if (
                        self.monitors.make_monitor(
                            device_id, output_id, current_cycles
                        )
                        ...
                        wx.MessageBox(
                            wx.GetTranslation(
                                "Failed to add monitor for {signal_name}"
                            ).format(signal_name=signal_name),
                            wx.GetTranslation("Error"),
                            wx.OK | wx.ICON_ERROR
                        )
    ...
    def on_add_all_monitors(self, event, dialog):
        ...
        dlg = wx.MessageDialog(
                dialog,
                wx.GetTranslation(
                    "Add all {x} available signals?"
                ).format(x=len(non_monitored)),
    ...
    def on_remove_monitor(self, event):
        ...
        if dlg.ShowModal() == wx.ID_YES:
            ...
            self.SetStatusText(
                wx.GetTranslation(
                    "Zapped all {monitor_count} monitors"
                ).format(monitor_count=monitor_count))
        dlg.Destroy()
    ...
    def update_switch_list(self):
        ...
        for i, switch_id in enumerate(switch_ids):
            ...
            self.switch_list.SetItemTextColour(
                index,
    ...
    def on_switch_selected(self, event):
        ...
        self.toggle_switch_btn.Enable(
            self.switch_list.GetSelectedItemCount() > 0
        )
    ...
    def on_toggle_switch(self, event):
        ...
        for selection in selected:
            ...
            # Toggle state
            new_state = (
                self.devices.LOW if current_state ==
                wx.GetTranslation("HIGH") else self.devices.HIGH
            )

            if self.devices.set_switch(device_id, new_state):
                self.SetStatusText(wx.GetTranslation(
                    "Toggled {switch_name} to {new_state}").format(
                        switch_name=switch_name, new_state=new_state))
            else:
                wx.MessageBox(
                    wx.GetTranslation(
                        "Failed to toggle switch {switch_name}"
                    ).format(switch_name=switch_name),
                    wx.GetTranslation("Error"),
                    wx.OK | wx.ICON_ERROR
                )
        ...
        if self.network.execute_network():
            ...
            wx.MessageBox(
                "Error: Network oscillating", "Error",
                wx.OK | wx.ICON_ERROR
                )
    ...
    def on_all_on(self, event):
        ...
        if self.network.execute_network():
            ...
            wx.MessageBox(
                wx.GetTranslation("Error: Network oscillating"),
                wx.GetTranslation("Error"),
                wx.OK | wx.ICON_ERROR
            )
    ...
    def on_all_off(self, event):
        ...
        if self.network.execute_network():
            ...
            wx.MessageBox(
                wx.GetTranslation("Error: Network oscillating"),
                wx.GetTranslation("Error"), wx.OK | wx.ICON_ERROR
                )
    ...
    def apply_theme(self):
        ...
        self.run_button.SetBackgroundColour(
            self.current_theme['button']['run'])
        self.stop_button.SetBackgroundColour(
            self.current_theme['button']['stop'])
        self.reset_button.SetBackgroundColour(
            self.current_theme['button']['reset'])
        self.continuous_button.SetBackgroundColour(
            self.current_theme['button']['continuous'])
        for btn in [self.run_button, self.stop_button,
                    self.reset_button, self.continuous_button]:
        ...
        # Apply to lists - only set background colors,
        # let text inherit from theme
        ...
        for box in [self.sim_box, self.switch_box, self.monitor_box]:
            box.SetBackgroundColour(
                self.current_theme['static_box']['background'])
        ...
        self.cycles_spin.SetBackgroundColour(
            self.current_theme['list']['background'])
    ...
    def on_monitor_selected(self, event):
        ...
        if index != -1:
            ...
            self.remove_monitor_btn.Enable(
                self.monitor_list.GetSelectedItemCount() > 0)
        event.Skip()
