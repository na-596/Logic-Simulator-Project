...
class Gui(wx.Frame):
    ...
    def on_menu(self, event):
        ...
        if Id == wx.ID_HELP:
            wx.MessageBox(
                wx.GetTranslation("How to Use the Logic Simulator GUI\n"
    "\n"
    "---\n"
    "\n"
    "**Keybinds**\n"
    ...
    def on_add_monitor(self, event):
        ...
        if (dialog.ShowModal() == wx.ID_OK and
    ...
    def on_toggle_switch(self, event):
        ...
        for selection in selected:
            ...
            # Toggle state
            if current_state == wx.GetTranslation("HIGH"):
                new_state = self.devices.LOW
            else:
            ...
            if self.devices.set_switch(device_id, new_state):
                self.SetStatusText(
                    wx.GetTranslation(
                        "Toggled {switch_name} to {new_state}"
                    ).format(switch_name=switch_name, new_state=new_state)
                )
            else:
                wx.MessageBox(
                    f"Failed to toggle switch {switch_name}", "Error",
                    wx.OK | wx.ICON_ERROR
                )
